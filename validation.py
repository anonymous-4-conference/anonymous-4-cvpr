...
def validate_epoch(
    ...
    with torch.no_grad():
        for i_batch, sampled_batch in tqdm(
            ...
            one_hot_pred = sampled_batch["label"].cuda()
            ...
            for j in range(len(one_hot_pred)):
                ...
                total_accuracy += accuracy
                if num_classes==4:
                    crystal_mean_sensitivity, crystal_overall_accuracy, crystal_f1_score, crystal_jaccard_index, crystal_f2_score, crystal_precision = calculate_metrics(one_hot_pred[j], volume_label[j], _class=3)
                    liquor_mean_sensitivity, liquor_overall_accuracy, liquor_f1_score, liquor_jaccard_index, liquor_f2_score, liquor_precision = calculate_metrics(one_hot_pred[j], volume_label[j], _class=1)
                    loop_mean_sensitivity, loop_overall_accuracy, loop_f1_score, loop_jaccard_index, loop_f2_score, loop_precision = calculate_metrics(one_hot_pred[j], volume_label[j], _class=2)
                    back_mean_sensitivity, back_overall_accuracy, back_f1_score, back_jaccard_index, back_f2_score, back_precision = calculate_metrics(one_hot_pred[j], volume_label[j], _class=0)
                    print(f'dataset: {image_path[j]}')
            ...
            for j in range(len(output)):
                    ...
                    total_accuracy += accuracy
                    if num_classes==4:
                        crystal_mean_sensitivity, crystal_overall_accuracy, crystal_f1_score, crystal_jaccard_index, crystal_f2_score, crystal_precision=calculate_metrics(output[j], volume_label[j],_class=3)
                        liquor_mean_sensitivity, liquor_overall_accuracy, liquor_f1_score, liquor_jaccard_index, liquor_f2_score, liquor_precision=calculate_metrics(output[j], volume_label[j],_class=1)
                        loop_mean_sensitivity, loop_overall_accuracy, loop_f1_score, loop_jaccard_index, loop_f2_score, loop_precision=calculate_metrics(output[j], volume_label[j],_class=2)
                        back_mean_sensitivity, back_overall_accuracy, back_f1_score, back_jaccard_index, back_f2_score, back_precision=calculate_metrics(output[j], volume_label[j],_class=0)
                        print(f'dataset: {image_path[j]}')
                        print(f'crystal_mean_sensitivity: {crystal_mean_sensitivity:.4f}, liquor_mean_sensitivity: {liquor_mean_sensitivity:.4f}, loop_mean_sensitivity: {loop_mean_sensitivity:.4f}, back_mean_sensitivity: {back_mean_sensitivity:.4f}')
                        print(f'crystal_overall_accuracy: {crystal_overall_accuracy:.4f}, liquor_overall_accuracy: {liquor_overall_accuracy:.4f}, loop_overall_accuracy: {loop_overall_accuracy:.4f}, back_overall_accuracy: {back_overall_accuracy:.4f}')
                        print(f'crystal_f1_score: {crystal_f1_score:.4f}, liquor_f1_score: {liquor_f1_score:.4f}, loop_f1_score: {loop_f1_score:.4f}, back_f1_score: {back_f1_score:.4f}')
        ...
        # pdb.set_trace()
            ...
            save_dir=f'./test_{dataset}/{dataset}_{save_name}/after_sam_rs_new_crrun'
          
            os.makedirs(save_dir,exist_ok=True)
            file_name = os.path.join(save_dir,f'./{dataset}_sam_pred.h5')
            # if '21116' in image_path[0]:
            #     previous=os.path.join(save_dir,f'21116_real_refine_pred_mask.npy')
            # if '19835' in image_path[0]:
            #     previous=os.path.join(save_dir,f'19835_real_refine_pred_mask.npy')
            # previous_data=np.load(previous)
            
            
            # with h5py.File(file_name, 'w') as h5_file:
            #     h5_file.create_dataset('train', data=previous_data)
            #     if label_original is not None:
            #         h5_file.create_dataset('label', data=label_original)
            # pdb.set_trace()
            # continue
            ...
            # for i in [1050]:
                ...
                cr_masks_1 =pred_cr_masks.copy()    
                
                open_kernel =np.ones((10,10),np.uint8)
                cr_masks_1 =cv2.morphologyEx(cr_masks_1, cv2.MORPH_OPEN, open_kernel)
                ...
                try:
                    cr_masks_1[cr_masks_1!=0]=20
                    lo_masks[lo_masks!=0]=30
                    final_mask = np.maximum.reduce([cr_masks_1, overall_li_masks, lo_masks])
                    final_mask[final_mask==20]=3
                    final_mask[final_mask==30]=2
                except:
            ...
            #    cr_input_points,cr_bbox= get_points(pred_mask_original[i],value=3)
                liquor_points,li_bbox= get_points(final_mask[0],value=1,Erosion=True)
                loop_points,lo_bbox= get_points(final_mask[0],value=2,Opening=True,kernel_size=(10,10))
                # back_points,back_bbox= get_points(label[i],value=0)
                cr_input_points,cr_bbox= get_points(final_mask[0],value=3)
                # hull = ConvexHull(liquor_points)

                # # Plotting
                # plt.plot(liquor_points[:, 0], liquor_points[:, 1], 'o')
                # for simplex in hull.simplices:
                #     plt.plot(liquor_points[simplex, 0], liquor_points[simplex, 1], 'k-')

                # plt.show()

                # Get the coordinates of the edge points
                # liquor_edge_points = liquor_points[hull.vertices]
                new_img[overall_li_masks[0]==0]=0
                new_img[final_mask[0]==2]=0
                ...
                if len(cr_input_points)<100:
                    ...
                    #
                    cr_masks_1,_,_= postprocess_sam_cr(predictor,new_img,cr_input_points,liquor_points,loop_points,cr_bbox,in_number=10,bb_scale_factor=1,crystal=False) #v,plot=True,
                cr_masks_1 = cr_masks_1*3 #liquor_points
                

                # cr_masks_1 = pred_cr_masks.copy()*3

                # close_kernel=np.ones((50,50),np.uint8)
                # cr_masks_1 =cv2.morphologyEx(cr_masks_1, cv2.MORPH_OPEN, close_kernel)
                # cr_masks_1 =cv2.morphologyEx(cr_masks_1, cv2.MORPH_CLOSE, close_kernel)
            ...
            # pdb.set_trace()
                ...
                if i%100==0:
                    ...
                    # np.save(f'{save_dir}/{dataset}_refine_pred_mask_{i}.npy', refine_pred_mask)
...
if __name__ == "__main__":
    ...
    model_save_path='/mnt/data_smaller/yishun/anacornew/raw_128_rs_2222b_2_onlysyn3/anacornet_iter_33_0.1434.pth'
    model_save_path='/mnt/data_smaller/yishun/anacornew/raw_128_rs_2222b_2_onlysyn3/anacornet_iter_32_0.1486.pth'
    
    region_size=[128,128,128]
    model_anacor =  create_model(region_size=region_size,resize_size=args.resize_size,depths=[3,3,3,3])
    model_save_path='/mnt/data_smaller/yishun/anacornew/raw_128_rs_3333b_6_all/anacornet_iter_22_0.2738.pth'
    ...
    db_validation = Tomodataset(
        ...
        test=False,
        test_dataset=['21116_real','19835_real',"20048_real", "21944_real"],#['21116_real','19835_real'], #['21116_real','13295_real','16846_real','20072_real'],#,'20467_sirt'], #['21116_sirt','13295_sirt','16846_sirt']  ,'20467_real' ,'19835_real','20453_real' '16846_real','13295_real','21116_real',"20048_real" "21944_real"
