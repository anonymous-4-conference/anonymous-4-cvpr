...
parser.add_argument("--save-dir", type=str, default="/mnt/data_smaller/yishun/anacor_net/", help="model_name")
parser.add_argument(
...
def create_model(region_size=(96, 96, 96),resize_size=128,depths=[3,3,3,3]):
    ...
    input_size=[int(region_size[0]/patch_size[0]) * int(region_size[1]/patch_size[1]) * int(region_size[2]/patch_size[2]),
                int(region_size[0]/patch_size[0]/2) * int(region_size[1]/patch_size[1]/2) * int(region_size[2]/patch_size[2]/2),
                int(region_size[0]/patch_size[0]/4) * int(region_size[1]/patch_size[1]/4) * int(region_size[2]/patch_size[2]/4),
                int(region_size[0]/patch_size[0]/8) * int(region_size[1]/patch_size[1]/8) * int(region_size[2]/patch_size[2]/8)]
...
def validate_epoch(
    model, validation_loader, writer, epoch_num, save_dir, region_size, num_classes,plot=True
):
    model.eval()
    ...
    with torch.no_grad():
        for i_batch, sampled_batch in tqdm(
            ...
            for j in range(len(output)):
                ...
                logging.info(f'crystal_accuracy: {crystal_accuracy*100:.2f}, liquor_accuracy: {liquor_accuracy*100:.2f}, loop_accuracy: {loop_accuracy*100:.2f}, back_accuracy: {back_accuracy*100:.2f}')
                logging.info(f'crystal_dice_loss: {dice_loss_crystal:.4f}, liquor_dice_loss: {dice_loss_liquor:.4f}, loop_dice_loss: {dice_loss_loop:.4f}, back_dice_loss: {dice_loss_back:.4f}')
                logging.info(f'cross_entropy_loss: {cross_entropy_loss:.4f}')
                total_crystal_accuracy+=crystal_accuracy
                total_accuracy+=(crystal_accuracy+liquor_accuracy+loop_accuracy)/3
                total_dice_loss += (dice_loss_crystal+dice_loss_liquor+dice_loss_loop)/3
            # pdb.set_trace()
...
if __name__ == "__main__":
    ...
    model_ = create_model(region_size=region_size,resize_size=args.resize_size,depths=[2,2,2,2])
    ...
    train_classes ={'crystal':3, 'background':0, 'loop':2, 'liquor':1}
    input_interpolation=0 # 0: nearest, 1: linear, 2:  Quadratic spline, 3: cubic spline
    # train_classes ={'crystal':3, 'background':0,}
    # train_classes ={'crystal':3, 'background':0,'loop':2}
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(model_optimizer, mode='min', factor=0.1, patience=5, verbose=True)
    ...
    for epoch_num in range(max_epoch):
        ...
        for i_batch, sampled_batch in tqdm(
            ...
            del model_input
            torch.cuda.empty_cache()
            class_weights = torch.tensor([1,1,1,1]).cuda()
            if num_classes==4:
                model_loss_seg_dice = DiceFocalLoss(softmax=True, weight=class_weights)(model_logits, model_label)
            else:
                model_loss_seg_dice = DiceFocalLoss(softmax=True)(model_logits, model_label)
            loss_total = model_loss_seg + model_loss_seg_dice
            
            predictions = torch.argmax(F.softmax(model_logits, dim=1), dim=1)
            last_channel_pred = (predictions == (num_classes - 1)).float()  # Convert to float for IoU calculation
            last_channel_label = (torch.argmax(model_label, dim=1) == (num_classes - 1)).float()
            ...
            current_iteration = (epoch_num+1) * len(trainloader)*batch_size + (i_batch+1)*batch_size
