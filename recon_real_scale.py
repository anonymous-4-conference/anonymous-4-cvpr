...
parser.add_argument(
    "--proj-dirs",
    type=str,
    default='/mnt/data/yishun/simulation/8th_batch/projections_real_scale_1/',
    help="The ending point of the batch",
)
parser.add_argument(
    "--recon_dirs",
    type=str,
    default='/mnt/data/yishun/simulation/8th_batch/recon_real_scale_thesis',
    help="The ending point of the batch",
)
parser.add_argument(
    "--dataset_list",
    type=str,
    default='19224 19485 19413',
    help="The ending point of the batch",
)
global args
...
if __name__ =="__main__":
    ...
    # slices_dirs='/mnt/data_smaller/yishun/real_data/tomopy/training/label/'

    # background_dirs='/mnt/data_smaller/yishun/simulation/background'

    # projection_dirs='/mnt/data/yishun/simulation/8th_batch/projections_real_scale_arc/'
    # recon_dirs='/mnt/data/yishun/simulation/8th_batch/recon_real_scale_arc'
    
    background_dirs=args.background_dirs
    slices_dirs=args.slices_dirs
    projection_dirs=args.proj_dirs
    ...
    required_dataset=['13295'] # ['13076','16846','13284','13304','13668','16000','21116'] #'13295',
    required_dataset=['19224','19485','19413']  #,'13295','16846','16115','19505']
    
    required_dataset=args.dataset_list.split(' ')
    if required_dataset[0] != 'None':
        slices_list=[path for path in slices_list if path not in not_required_dataset and path in required_dataset]
    else:
        slices_list=[path for path in slices_list if path not in not_required_dataset]
    ...
    wavelength='21116'
    dataset=wavelength
    # required_scale=#0.01,0.1,0.05,
    required_scale=[1,0.5,0.01,0.1,0.05,]  #
    required_dataset=['20072','19835']
    required_dataset=['13304'] 
    required_scale.reverse()
    ...
    for scale in projection_dirs_list:
        ...
        for i,proj_dirs in enumerate(scale_proj_list):
            ...
            if dataset not in required_dataset:
                continue
            with open(os.path.join(scale_proj_dirs,meta_data)) as f:
                meta_data = json.load(f)
            try:
                background = meta_data[proj_dirs]['background']

            except:
                print(f'background is not in {proj_dirs}')
                background = "20518"
            ...
            try:
                assert slices == proj_dirs
            except:
                print(f'error: {slices} and {proj_dirs} are not the same')
                pdb.set_trace()
            ...
            reconstruction(img_list,options,file_name,center=None,slicemodel=model2,syn=False)
            t2 = time.time()
